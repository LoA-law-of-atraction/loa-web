...
export default function ManageCameraMovementsPage() {
  ...
  const loadItems = async () => {
    ...
    try {
      ...
      if (result.success) setItems(result.camera_movements || []);
    } catch (error) {
      console.error("Error loading camera movements:", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSeed = async () => {
    setSeeding(true);
    try {
      const response = await fetch("/api/camera-movements/seed", {
        method: "POST",
      });
      const result = await response.json();
      if (result.success) {
        await alert(result.message || "Seeded camera movements", "success");
        loadItems();
      } else {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        await alert(
          `✨ Generated ${result.count || 0} camera movement(s) with AI${cost ? ` (cost: $${cost.toFixed(4)})` : ""}`,
          "success",
        );
        setShowGenerateModal(false);
        setGenerateKeywords("");
  ...
  const handleCreate = async () => {
    ...
    try {
      const response = await fetch("/api/camera-movements/create", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      const result = await response.json();
      if (result.success) {
        await alert("Camera movement created", "success");
        handleCloseModals();
        loadItems();
      } else {
  ...
  const handleUpdate = async () => {
    ...
    try {
      const response = await fetch(`/api/camera-movements/${editing.id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          name: formData.name.trim(),
      ...
      const result = await response.json();
      if (result.success) {
        await alert("Camera movement updated", "success");
        handleCloseModals();
        loadItems();
      } else {
  ...
  const handleDelete = async (id) => {
    const ok = confirm("Delete this camera movement? This cannot be undone.");
    if (!ok) return;
    try {
      const response = await fetch(`/api/camera-movements/${id}`, {
        method: "DELETE",
      });
      const result = await response.json();
      if (result.success) {
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-600 dark:text-gray-300">
            Configure the camera movement library used in Step 4.
          </p>
        </div>
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            className="admin-btn-secondary"
            title="Generate camera movements with AI"
          >
            ✨ Generate
          </button>
      ...
      <div className="admin-card-solid p-4">
        {loading ? (
          ...
          <div className="text-sm admin-muted">
            No camera movements yet. Click &quot;Seed defaults&quot;.
          </div>
        ) : (
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-gray-950 rounded-2xl shadow-xl w-full max-w-md overflow-hidden">
            <div className="p-5 border-b border-gray-200 dark:border-gray-800">
              <div className="flex items-center justify-between gap-4">
                <div className="font-semibold text-gray-900 dark:text-gray-100">
                  Generate camera movements with AI
                </div>
                <button
                  onClick={handleCloseModals}
                  className="text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200"
              ...
              <div className="text-sm text-gray-600 dark:text-gray-300 mt-1">
                Provide keywords like “slow push-in, handheld, gentle drift”.
              </div>
            </div>
            ...
            <div className="p-5 space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="e.g., slow push-in, lock-off, subtle pan"
                  className="w-full mt-1 px-3 py-2 rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-950 text-gray-900 dark:text-gray-100"
                  disabled={generating}
                />
              ...
              <div>
                ...
                >
                  ...
                  <option value={3}>3 movements</option>
