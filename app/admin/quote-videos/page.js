...
import { MessageSquare, Image as ImageIcon, Video, Music2, Layers, Share2, Plus, Trash2, Info, Download, Maximize2 } from "lucide-react";
...
function QuoteVideosContent() {
  ...
  const [currentProjectId, setCurrentProjectId] = useState(urlProjectId || null);
  ...
  const [textToImageModel, setTextToImageModel] = useState(DEFAULT_QUOTE_VIDEOS_IMAGE_MODEL);
  const [referenceImageUrl, setReferenceImageUrl] = useState("");
  const [uploadingReferenceImage, setUploadingReferenceImage] = useState(false);
  const [generatingPromptFromImage, setGeneratingPromptFromImage] = useState(false);
  const [showReferenceGalleryModal, setShowReferenceGalleryModal] = useState(false);
  const [showSceneImageGalleryModal, setShowSceneImageGalleryModal] = useState(false);
  ...
  const [deletingBackgroundImageUrl, setDeletingBackgroundImageUrl] = useState(null);
  ...
  const [videoHistoryTargetSceneId, setVideoHistoryTargetSceneId] = useState(null);
  const [globalVideoHistoryEntries, setGlobalVideoHistoryEntries] = useState([]);
  const [globalVideoHistoryLoading, setGlobalVideoHistoryLoading] = useState(false);
  const [musicUrl, setMusicUrl] = useState("");
  const [generatingVideo, setGeneratingVideo] = useState(false);
  const [generatingVideoSceneId, setGeneratingVideoSceneId] = useState(null);
  const [generatingVideoPromptSceneId, setGeneratingVideoPromptSceneId] = useState(null);
  ...
  const [generatingBackgroundMusic, setGeneratingBackgroundMusic] = useState(false);
  ...
  const [availableMusicFromCollection, setAvailableMusicFromCollection] = useState([]);
  const [loadingMusicCollection, setLoadingMusicCollection] = useState(false);
  const [showMusicCollectionPicker, setShowMusicCollectionPicker] = useState(false);
  ...
  const loadThemeSuggestions = useCallback(async (query = "") => {
    try {
      ...
      const data = await res.json();
      if (data.success && Array.isArray(data.suggestions)) setThemeSuggestions(data.suggestions);
    } catch (e) {
      console.error(e);
    }
  }, []);
  ...
  const loadProject = useCallback(async (id, options = {}) => {
    ...
    try {
      ...
      if (data.success && data.project) {
        ...
        setTextToImageModel(
          QUOTE_VIDEOS_TEXT_TO_IMAGE_MODELS.some((m) => m.id === p.text_to_image_model)
        ...
        setDurationSeconds(
          p.duration_seconds ??
            (firstSceneWithVideo?.duration != null ? firstSceneWithVideo.duration : null) ??
            8
        );
        ...
        setGrainOpacity(typeof p.grain_opacity === "number" ? Math.max(0, Math.min(1, p.grain_opacity)) : 0.2);
        if (!keepStep) {
          const savedStep = typeof p.current_step === "number" && p.current_step >= 1 && p.current_step <= 6
            ? p.current_step
            : 1;
          setStep(savedStep);
        }
  ...
  useEffect(() => {
    fetch("/api/quote-videos/grains-list")
      .then((r) => r.json())
      .then((data) => {
        if (data.files && Array.isArray(data.files)) setLocalGrainsList(data.files);
      })
  ...
  const loadMusicPricing = useCallback((modelId) => {
    setLoadingMusicPricing(true);
    fetch(`/api/video-generator/music-pricing?model_id=${encodeURIComponent(modelId || selectedMusicModelId)}`)
      .then((r) => r.json())
      .then((data) => {
        if (data.success) {
      ...
      .catch(() => { setMusicUnitCost(null); setMusicPricingUnit(null); })
  ...
  const loadAvailableMusicFromCollection = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/video-generator/music-list?project_id=${currentProjectId}&limit=50`);
      const data = await res.json();
  ...
  }, [step, currentProjectId, selectedMusicModelId, loadMusicPricing, loadAvailableMusicFromCollection]);
  ...
  useEffect(() => {
    ...
    if (justEnteredStep || switchedProject) {
      ...
      const totalSec =
        scenes.length > 0
          ? scenes.reduce((s, sc) => s + Math.max(1, Math.min(15, Math.round(Number(sc.duration) || 8))), 0)
  ...
  useEffect(() => {
    if (musicBasePrompts.length > 0) {
      const selected = musicBasePrompts.find((b) => b.id === selectedMusicBaseId) || musicBasePrompts[0];
  ...
  useEffect(() => {
    ...
    fetch(`/api/quote-videos/projects/${currentProjectId}/generated-images`)
      .then((r) => r.json())
      .then((data) => {
        if (data.success && Array.isArray(data.images)) setGalleryImages(data.images);
  ...
  useEffect(() => {
    ...
    fetch("/api/quote-videos/video-history")
      .then((r) => r.json())
      .then((data) => {
        if (data.success && Array.isArray(data.entries)) setGlobalVideoHistoryEntries(data.entries);
  ...
  const handleGenerateQuote = async () => {
    ...
    try {
      const res = await fetch("/api/quote-videos/generate-quote", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ theme: theme.trim(), project_id: currentProjectId || undefined }),
      });
      const data = await res.json();
  ...
  const addTranscriptSentencesAsQuotes = () => {
    ...
    if (currentProjectId) {
      fetch(`/api/quote-videos/projects/${currentProjectId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ quote_list: nextList, quote_text: sentences[0] || quoteText }),
      }).catch(console.error);
    }
  };
  ...
  const addFullTranscriptAsQuote = () => {
    if (!youtubeTranscript.trim()) return;
    const text = youtubeTranscript.length > 500 ? youtubeTranscript.slice(0, 500).trim() : youtubeTranscript;
  ...
  const handleAIPickQuotes = async () => {
    ...
    try {
      const res = await fetch("/api/quote-videos/extract-quotes", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ transcript: youtubeTranscript, project_id: currentProjectId || undefined }),
      });
      const data = await res.json();
      if (data.success && Array.isArray(data.quotes) && data.quotes.length > 0) {
        ...
        if (currentProjectId) {
          await fetch(`/api/quote-videos/projects/${currentProjectId}`, {
            method: "PATCH",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ quote_list: nextList, quote_text: data.quotes[0] }),
          });
  ...
  const uploadReferenceFile = async (file) => {
    if (!file || !file.type.startsWith("image/")) {
      await alert("Please use an image file (JPEG, PNG, WebP, or GIF).", "warning");
      return;
    }
    ...
    try {
      ...
      if (data.success && data.url) {
        ...
        if (currentProjectId) {
          let history = [];
          try {
            const resGet = await fetch(`/api/quote-videos/projects/${currentProjectId}`);
            const dataGet = await resGet.json();
            if (dataGet.success && dataGet.project && Array.isArray(dataGet.project.reference_image_history)) {
              history = dataGet.project.reference_image_history;
            }
          } catch (_) {}
          const next = [...history, { url: data.url, path: data.path || null, created_at: new Date().toISOString() }].slice(-30);
          await fetch(`/api/quote-videos/projects/${currentProjectId}`, {
            method: "PATCH",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
  ...
  const handleReferenceDragLeave = (e) => {
    e.preventDefault();
    if (!e.currentTarget.contains(e.relatedTarget)) setReferenceDropActive(false);
  };
  ...
  const handleImageToPrompt = async () => {
    ...
    try {
      ...
      if (data.success && data.base_prompt) {
        ...
        const finalPrompt = (p.toLowerCase().includes("no text") && p.toLowerCase().includes("9:16") ? p : p + suffix);
        setImagePromptUsed(finalPrompt);
        if (project?.id) {
          const history = Array.isArray(project.reference_image_history) ? project.reference_image_history : [];
          const updatedHistory = history.map((entry) => {
            const entryUrl = typeof entry === "string" ? entry : entry?.url;
            if (entryUrl === url) {
              const base = typeof entry === "object" && entry != null ? { ...entry } : { url: entryUrl };
  ...
  const updateProjectCurrentStep = useCallback(async (nextStep) => {
    if (!currentProjectId || nextStep < 1 || nextStep > 6) return;
    try {
      const res = await fetch(`/api/quote-videos/projects/${currentProjectId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  const handleGenerateImage = async () => {
    ...
    try {
      ...
      const idx = scenes.length > 0 ? Math.min(currentSceneIndex, Math.max(0, scenes.length - 1)) : -1;
      ...
      if (data.success && data.background_image_url) {
        ...
        await alert(data.error || data.message || "Generate image failed", "error");
      }
    } catch (e) {
      await alert("Error: " + e.message, "error");
    } finally {
  ...
  const handleAddScene = async () => {
    ...
    try {
      if (scenes.length === 0) {
        const hist = Array.isArray(project.background_image_history) ? project.background_image_history : [];
        const image_urls = hist.map((e) => (typeof e === "string" ? e : e?.url)).filter(Boolean);
        const image_prompts = hist.map((e) => (typeof e === "object" && e?.prompt_sent_to_model ? e.prompt_sent_to_model : "")).slice(0, image_urls.length);
        const image_metadata = hist.map((e) => typeof e === "object" && e ? { prompt_sent_to_model: e.prompt_sent_to_model ?? null, model_endpoint: e.model_endpoint ?? null, fal_request_payload: e.fal_request_payload ?? null, created_at: e.created_at ?? null } : {}).slice(0, image_urls.length);
        ...
        const res1 = await fetch(`/api/quote-videos/projects/${currentProjectId}/scenes`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            label: "Scene 1",
            ...
            image_urls,
            image_prompts: image_prompts.length === image_urls.length ? image_prompts : image_urls.map(() => ""),
            selected_image_index,
            image_metadata: image_metadata.length === image_urls.length ? image_metadata : [],
          }),
        });
        const data1 = await res1.json();
        if (!data1.success) throw new Error(data1.error || "Failed to create Scene 1");
        const res2 = await fetch(`/api/quote-videos/projects/${currentProjectId}/scenes`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ label: "Scene 2" }),
        });
        const data2 = await res2.json();
        if (!data2.success) throw new Error(data2.error || "Failed to create Scene 2");
        setCurrentSceneIndex(1);
        setBackgroundImageUrl("");
        loadProject(currentProjectId);
      } else {
        const res = await fetch(`/api/quote-videos/projects/${currentProjectId}/scenes`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  const handleGenerateVideo = async () => {
    ...
    if (!backgroundImageUrl.trim() && !quoteText.trim() && !theme.trim()) {
      await alert("Generate an image first (step 2) or complete Quote step.", "warning");
      return;
    }
    ...
    try {
      ...
      if (data.success && data.animation_video_url) {
        ...
        await alert(data.error || data.message || "Generate video failed", "error");
      }
    } catch (e) {
      await alert("Error: " + e.message, "error");
    } finally {
  ...
  const handleSelectVideoFromHistoryForScene = async (entry, sceneId) => {
    ...
    try {
      const res = await fetch(`/api/quote-videos/projects/${currentProjectId}/scenes/${sceneId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  const handleApplyGrain = async (imageUrl, sceneId) => {
    ...
    try {
      ...
      if (data.success && data.url) {
        ...
        await alert(data.error || data.message || "Apply grain failed", "error");
      }
    } catch (e) {
      await alert("Error: " + e.message, "error");
    } finally {
  ...
  const handleSelectFromGallery = async (url) => {
    ...
    const safeSceneIndex = useScenes ? Math.min(currentSceneIndex, Math.max(0, scenes.length - 1)) : 0;
    const currentScene = useScenes ? scenes[safeSceneIndex] : null;
    try {
      if (useScenes && currentScene?.id) {
        const urls = Array.isArray(currentScene.image_urls) ? currentScene.image_urls : [];
        const prompts = Array.isArray(currentScene.image_prompts) ? currentScene.image_prompts : [];
        const meta = Array.isArray(currentScene.image_metadata) ? currentScene.image_metadata : [];
        const idx = urls.indexOf(url);
        if (idx >= 0) {
          await fetch(`/api/quote-videos/projects/${currentProjectId}/scenes/${currentScene.id}`, {
            method: "PATCH",
            headers: { "Content-Type": "application/json" },
  ...
  const handleGenerateVideoForScene = async (scene) => {
    ...
    try {
      ...
      if (data.success && data.videos?.length) {
        await loadProject(currentProjectId);
        const v = data.videos.find((x) => String(x.scene_id) === String(scene.id));
  ...
  const handleGenerateVideoPromptForScene = async (scene) => {
    if (!currentProjectId || !scene?.id) return;
    const selIdx = typeof scene.selected_image_index === "number" ? scene.selected_image_index : 0;
    ...
    const imagePromptText = (prompts[selIdx] && String(prompts[selIdx]).trim()) || meta[selIdx]?.prompt_sent_to_model || scene.image_prompt || "";
    if (!imagePromptText.trim()) {
      await alert("This scene has no image prompt. Generate or select an image first (Step 2).", "warning");
      return;
    }
    const ok = await confirm(
      `Generate a video prompt for ${scene.label || "Scene " + scene.id} from its image prompt?\n\nYou can review and edit before saving.`,
      { title: "Generate Video Prompt", confirmText: "Generate", cancelText: "Cancel" }
    );
    if (!ok) return;
    ...
    try {
      ...
      if (data.success && data.motion_prompt != null) {
        setVideoPromptReview({ sceneId: scene.id, generatedPrompt: data.motion_prompt, cost: data.cost ?? 0 });
        setVideoPromptReviewDraft(data.motion_prompt);
      } else {
        await alert(data.error || data.message || "Failed to generate video prompt", "error");
      }
    } catch (e) {
      await alert("Error: " + e.message, "error");
    } finally {
  ...
  const handleSaveVideoPromptReview = async () => {
    ...
    try {
      await fetch(`/api/quote-videos/projects/${currentProjectId}/scenes/${videoPromptReview.sceneId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ motion_prompt: videoPromptReviewDraft.trim() }),
      });
  ...
  const handleAssemble = async () => {
    ...
    const scenesWithVideo = rawScenes
      ...
      .map((s) => ({
        ...
        duration_seconds: Math.max(1, Math.min(60, Math.round(Number(s.duration) || 8))),
      }));
    const hasScenes = scenesWithVideo.length > 0;
    // DEBUG: Step 5 render – what we're sending from Step 3 selections
    console.log("[QuoteVideos Render] project.scenes (raw):", rawScenes.length, rawScenes.map((s, i) => ({
      index: i,
      id: s.id,
      label: s.label,
      selected_video_url: (s.selected_video_url || "").slice(0, 60) + ((s.selected_video_url || "").length > 60 ? "…" : ""),
      duration: s.duration,
    })));
    console.log("[QuoteVideos Render] scenesWithVideo (sent to assemble):", scenesWithVideo.length, scenesWithVideo.map((s, i) => ({
      index: i,
      video_url: (s.video_url || "").slice(0, 60) + ((s.video_url || "").length > 60 ? "…" : ""),
      duration_seconds: s.duration_seconds,
    })));
    console.log("[QuoteVideos Render] hasScenes:", hasScenes, "animationVideoUrl (single-scene):", (animationVideoUrl || "").slice(0, 60) + ((animationVideoUrl || "").length > 60 ? "…" : ""));
    if (!currentProjectId || !quoteText.trim()) {
      await alert("Fill in quote first", "warning");
      return;
    }
    if (!hasScenes && !animationVideoUrl.trim()) {
      await alert("Add at least one scene with a generated video (Step 3) before rendering", "warning");
      return;
    }
    const payload = {
      project_id: currentProjectId,
    ...
    if (hasScenes) {
      ...
      console.log("[QuoteVideos Render] payload.scenes count:", payload.scenes.length);
    } else {
      payload.animation_video_url = animationVideoUrl.trim();
      payload.duration_seconds = durationSeconds;
      console.log("[QuoteVideos Render] using single video (no scenes), duration_seconds:", payload.duration_seconds);
    }
    const currentTimelineSettings = timelineEditorRef.current?.getTimelineSettings?.();
    if (currentTimelineSettings && typeof currentTimelineSettings === "object" && Object.keys(currentTimelineSettings).length > 0) {
    ...
    try {
      const res = await fetch("/api/video-generator/post-social", {
        ...
        body: JSON.stringify({
          ...
          caption: (platform === "instagram" ? instagramCaption : platform === "youtube" ? youtubeCaption : tiktokCaption) || caption || quoteText || "",
      ...
      if (data.success) {
        ...
        let errMsg = data.message || data.error || `Failed to post to ${platform}`;
  ...
  const handleGenerateCaption = async (platform) => {
    ...
    try {
      const res = await fetch("/api/video-generator/generate-caption", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ topic: theme || "", script: quoteText, platform }),
      });
      const data = await res.json();
  ...
  const handleSwitchInstagramAccount = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        window.location.href = "/api/auth/instagram?return_to=" + encodeURIComponent(returnTo);
        return;
      }
      await alert(data.error || "Failed to disconnect", "error");
    } catch (e) {
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8">
          ...
          >
            <div className="w-14 h-14 rounded-2xl bg-blue-100 dark:bg-blue-500/20 flex items-center justify-center flex-shrink-0 group-hover:scale-105 transition-transform">
              <Plus className="w-7 h-7 text-blue-600 dark:text-blue-400" strokeWidth={2} />
            </div>
          ...
          {loadingProjects ? (
            ...
            <p className="text-gray-500 dark:text-gray-400 py-8">No projects yet. Create one above.</p>
          ) : (
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white dark:from-gray-950 dark:to-gray-900">
        ...
        <>
          {step === 1 && (
            <div className="space-y-6">
              <div className="mb-8">
                <h2 className="text-2xl font-bold tracking-tight text-gray-900 dark:text-gray-100">Step 1: Quote</h2>
                <div className="flex flex-wrap gap-2 mt-4">
                  <div className="bg-blue-50 border border-blue-200 dark:border-blue-800 px-3 py-1.5 rounded-lg text-xs sm:text-sm font-medium flex items-center gap-1.5 whitespace-nowrap dark:bg-blue-950/40">
                    <span className="text-blue-700 dark:text-blue-200">🤖 Model:</span>
                    <span className="font-semibold text-blue-900 dark:text-blue-100">
                      {process.env.NEXT_PUBLIC_QUOTE_CLAUDE_MODEL_LABEL || process.env.NEXT_PUBLIC_CLAUDE_MODEL || "—"}
                    </span>
                  </div>
                  <div className="bg-blue-50 border border-blue-200 dark:border-blue-800 px-3 py-1.5 rounded-lg text-xs sm:text-sm font-medium flex items-center gap-1.5 whitespace-nowrap dark:bg-blue-950/40">
                    <span className="text-blue-700 dark:text-blue-200">📥 Input:</span>
                    <span className="font-semibold text-blue-900 dark:text-blue-100">
                      ${(parseFloat(process.env.NEXT_PUBLIC_CLAUDE_INPUT_PER_MILLION || "3.00") / 1e6).toFixed(6)}/tok
                    </span>
                  </div>
                  <div className="bg-blue-50 border border-blue-200 dark:border-blue-800 px-3 py-1.5 rounded-lg text-xs sm:text-sm font-medium flex items-center gap-1.5 whitespace-nowrap dark:bg-blue-950/40">
                    <span className="text-blue-700 dark:text-blue-200">📤 Output:</span>
                    <span className="font-semibold text-blue-900 dark:text-blue-100">
                      ${(parseFloat(process.env.NEXT_PUBLIC_CLAUDE_OUTPUT_PER_MILLION || "15.00") / 1e6).toFixed(6)}/tok
                    </span>
                  </div>
                  {project?.costs?.step1?.claude > 0 && (
                    <div className="bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 px-3 py-1.5 rounded-lg text-xs sm:text-sm font-medium flex items-center gap-1.5 whitespace-nowrap">
                      <span className="text-gray-600 dark:text-gray-300">🤖 Claude:</span>
                      <span className="font-semibold text-gray-900 dark:text-gray-100">${project.costs.step1.claude.toFixed(4)}</span>
                    </div>
                  )}
                  {project?.costs?.step1?.total > 0 && (
                    <div className="bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 px-3 py-1.5 rounded-lg text-xs sm:text-sm font-medium flex items-center gap-1.5 whitespace-nowrap">
                      <span className="text-gray-600 dark:text-gray-300">💰 Step 1 Total:</span>
                      <span className="font-semibold text-gray-900 dark:text-gray-100">${project.costs.step1.total.toFixed(4)}</span>
                    </div>
                  )}
                </div>
              </div>
              <div className="relative">
                ...
                <input
                  ...
                  onFocus={() => {
                    if (themeBlurRef.current) clearTimeout(themeBlurRef.current);
                    setShowThemeDropdown(true);
                  }}
                  onBlur={() => {
                    themeBlurRef.current = setTimeout(() => setShowThemeDropdown(false), 180);
                  }}
                ...
                {showThemeDropdown && themeSuggestions.length > 0 && (
                  ...
                  >
                    {themeSuggestions
                      .filter((t) => !theme.trim() || t.toLowerCase().includes(theme.trim().toLowerCase()))
                      .slice(0, 12)
                      .map((s) => (
                        <li key={s}>
                          <button
              ...
              <div className="border-t border-gray-200 dark:border-gray-700 pt-6">
                ...
                {youtubeTranscriptError && (
                  <p className="mt-1 text-sm text-red-600 dark:text-red-400">{youtubeTranscriptError}</p>
                )}
                {youtubeTranscript && (
                  <div className="mt-3 space-y-2">
                    <p className="text-xs text-gray-500 dark:text-gray-400">Transcript (use below to add as quotes)</p>
                    ...
                    <div className="flex flex-wrap gap-2">
                      ...
                      >
                        {extractingQuotes ? "Picking…" : "AI pick punchy quotes"}
                      </button>
                      <button
                        type="button"
              ...
              {quoteList.length > 0 && (
                <div>
                  ...
                  <ul className="space-y-2 max-h-48 overflow-auto rounded-lg border border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-800/50 p-2">
                    {quoteList.map((q, i) => (
                      <li key={`${i}-${q.slice(0, 20)}`} className="flex items-center gap-2">
                        <button
                          type="button"
                        ...
                        <button
                          type="button"
                          onClick={async () => {
                            const nextList = quoteList.filter((_, j) => j !== i);
                            const newSelected = quoteText === q ? (nextList[0] ?? "") : quoteText;
                            setQuoteList(nextList);
          ...
          {step === 2 && (() => {
            ...
            <div className="space-y-6">
              <div className="mb-8">
                <h2 className="text-2xl font-bold tracking-tight text-gray-900 dark:text-gray-100">Step 2: Image</h2>
                <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Claude (reference → prompt) · Fal (image generation)</p>
                <div className="flex flex-wrap gap-2 mt-4">
                  <div className="bg-blue-50 border border-blue-200 dark:border-blue-800 px-3 py-1.5 rounded-lg text-xs sm:text-sm font-medium flex items-center gap-1.5 whitespace-nowrap dark:bg-blue-950/40">
                    <span className="text-blue-700 dark:text-blue-200">🤖 Claude (image→prompt):</span>
                    <span className="font-semibold text-blue-900 dark:text-blue-100">
                      {process.env.NEXT_PUBLIC_QUOTE_CLAUDE_MODEL_LABEL || process.env.NEXT_PUBLIC_CLAUDE_MODEL || "—"}
                    </span>
                  </div>
                  <div className="bg-blue-50 border border-blue-200 dark:border-blue-800 px-3 py-1.5 rounded-lg text-xs sm:text-sm font-medium flex items-center gap-1.5 whitespace-nowrap dark:bg-blue-950/40">
                    <span className="text-blue-700 dark:text-blue-200">📥 Claude Input:</span>
                    <span className="font-semibold text-blue-900 dark:text-blue-100">
                      ${(parseFloat(process.env.NEXT_PUBLIC_CLAUDE_INPUT_PER_MILLION || "3.00") / 1e6).toFixed(6)}/tok
                    </span>
                  </div>
                  <div className="bg-blue-50 border border-blue-200 dark:border-blue-800 px-3 py-1.5 rounded-lg text-xs sm:text-sm font-medium flex items-center gap-1.5 whitespace-nowrap dark:bg-blue-950/40">
                    <span className="text-blue-700 dark:text-blue-200">📤 Claude Output:</span>
                    <span className="font-semibold text-blue-900 dark:text-blue-100">
                      ${(parseFloat(process.env.NEXT_PUBLIC_CLAUDE_OUTPUT_PER_MILLION || "15.00") / 1e6).toFixed(6)}/tok
                    </span>
                  </div>
                  {project?.costs?.step2?.claude > 0 && (
                    <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-700 px-3 py-1.5 rounded-lg text-xs sm:text-sm font-medium flex items-center gap-1.5 whitespace-nowrap">
                      <span className="text-blue-700 dark:text-blue-200">🤖 Claude:</span>
                      <span className="font-semibold text-blue-900 dark:text-blue-100">${project.costs.step2.claude.toFixed(4)}</span>
                    </div>
                  )}
                  <div className="bg-purple-50 border border-purple-200 dark:border-purple-800 px-3 py-1.5 rounded-lg text-xs sm:text-sm font-medium flex items-center gap-1.5 whitespace-nowrap dark:bg-purple-950/40">
                    <span className="text-purple-700 dark:text-purple-200">🖼️ Fal (image gen):</span>
                  ...
                  {project?.costs?.step2?.fal > 0 && (
                    <div className="bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-700 px-3 py-1.5 rounded-lg text-xs sm:text-sm font-medium flex items-center gap-1.5 whitespace-nowrap">
                      <span className="text-purple-700 dark:text-purple-200">🖼️ Fal:</span>
                      <span className="font-semibold text-purple-900 dark:text-purple-100">${project.costs.step2.fal.toFixed(4)}</span>
                    </div>
                  )}
                  {project?.costs?.step2?.total > 0 && (
                    <div className="bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 px-3 py-1.5 rounded-lg text-xs sm:text-sm font-medium flex items-center gap-1.5 whitespace-nowrap">
                      <span className="text-gray-600 dark:text-gray-300">💰 Step 2 Total:</span>
                      <span className="font-semibold text-gray-900 dark:text-gray-100">${project.costs.step2.total.toFixed(4)}</span>
                    </div>
                  )}
                </div>
                <div className="mt-4 flex flex-wrap items-center gap-2">
                  <label htmlFor="text-to-image-model" className="text-sm font-medium text-gray-700 dark:text-gray-300">
                    Text-to-image model:
                  </label>
                  <select
                    ...
                    onChange={async (e) => {
                      ...
                      if (!QUOTE_VIDEOS_TEXT_TO_IMAGE_MODELS.some((m) => m.id === next) || !currentProjectId) return;
                      setTextToImageModel(next);
                      try {
                        await fetch(`/api/quote-videos/projects/${currentProjectId}`, {
                          method: "PATCH",
                          headers: { "Content-Type": "application/json" },
                          body: JSON.stringify({ text_to_image_model: next }),
                        });
                      } catch (err) {
                        console.error("Failed to save text-to-image model:", err);
                      }
                    }}
                    disabled={!project?.id}
              ...
              {(generatingImage || generatingPromptFromImage || loadingFinalPrompt || uploadingReferenceImage) && (
                <div className="rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900/80 p-4 shadow-sm">
                  ...
                  <div className="h-2 w-full rounded-full bg-gray-200 dark:bg-gray-700 overflow-hidden">
                    <div
                      ...
                      style={{ animation: "quote-step2-loading-bar 1.5s ease-in-out infinite" }}
                    />
                  </div>
                </div>
              )}
              ...
              <div className="rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900/80 p-5 shadow-sm">
                <p className="text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400 mb-1">Quote for this image</p>
                <p className="text-gray-800 dark:text-gray-200 italic">&ldquo;{quoteText || "—"}&rdquo;</p>
              </div>
              {/* Section: Reference image + Image to prompt */}
              <div className="rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900/80 p-5 shadow-sm">
                <p className="text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400 mb-3">Image to prompt</p>
                <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
                  Upload a reference image. Claude analyzes it to create a scene prompt only (image is not used for generation). 9:16.
                </p>
                <div
                ...
                >
                  <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                    ...
                    >
                      {referenceDropActive ? (
                        <p className="text-base font-semibold text-blue-600 dark:text-blue-400">Drop image here</p>
                      ) : (
                        <p className="text-sm text-gray-600 dark:text-gray-400">Drag and drop or use the buttons below.</p>
                      )}
                    </div>
                    <input
                    ...
                    <div className="flex flex-wrap items-center gap-2 shrink-0">
                      ...
                      >
                        {uploadingReferenceImage ? "Uploading…" : "Upload"}
                      </button>
                      <button
                        type="button"
                ...
                {referenceImageUrl && (
                  <div className="mt-4 flex flex-wrap items-center gap-4">
                    ...
                    >
                      {generatingPromptFromImage ? "Analyzing…" : "Image to prompt"}
                    </button>
                  </div>
                )}
              </div>
              ...
              <div className="rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900/80 p-5 shadow-sm">
                <p className="text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400 mb-2">Image prompt</p>
                <p className="text-sm text-gray-700 dark:text-gray-300 font-mono whitespace-pre-wrap break-words leading-relaxed">
                  {imagePromptUsed.trim() ? imagePromptUsed : "Use “Image to prompt” above to generate the prompt from your reference image."}
                </p>
              </div>
              ...
              {imagePromptUsed.trim() && (
                <div className="rounded-2xl border border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-900/50 p-5 shadow-sm">
                  <p className="text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400 mb-2">Final prompt (dark Ghibli)</p>
                  {loadingFinalPrompt ? (
              ...
              <div className="rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900/80 p-5 shadow-sm">
                <p className="text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400 mb-2">Generate from final prompt</p>
                <button
                  type="button"
                ...
                >
                  {generatingImage ? "Generating image…" : "Generate new image"}
                </button>
              </div>
              {(() => {
                const scenes = Array.isArray(project?.scenes) ? project.scenes : [];
                ...
                function getSceneImageState(sceneOrProject, isScene) {
                  if (!sceneOrProject) return { image_urls: [], selected_index: 0, entries: [] };
                  const urls = Array.isArray(sceneOrProject.image_urls) ? sceneOrProject.image_urls : [];
                  if (urls.length > 0) {
                    const meta = Array.isArray(sceneOrProject.image_metadata) ? sceneOrProject.image_metadata : [];
                    const idx = typeof sceneOrProject.selected_image_index === "number" ? Math.max(0, Math.min(sceneOrProject.selected_image_index, urls.length - 1)) : 0;
                    const entries = urls.map((url, i) => ({
                      url,
                      created_at: meta[i]?.created_at ?? null,
                      prompt_sent_to_model: meta[i]?.prompt_sent_to_model ?? null,
                      model_endpoint: meta[i]?.model_endpoint ?? null,
                      fal_request_payload: meta[i]?.fal_request_payload ?? null,
                    }));
                    return { image_urls: urls, selected_index: idx, entries };
                  }
                  const hist = Array.isArray(sceneOrProject.background_image_history) ? sceneOrProject.background_image_history : [];
                  const legacyUrls = hist.map((e) => (typeof e === "string" ? e : e?.url)).filter(Boolean);
                  const legacyEntries = hist.map((e) => {
                    if (typeof e === "string") return { url: e, created_at: null, prompt_sent_to_model: null, model_endpoint: null, fal_request_payload: null };
                    return { url: e?.url, created_at: e?.created_at ?? null, prompt_sent_to_model: e?.prompt_sent_to_model ?? null, model_endpoint: e?.model_endpoint ?? null, fal_request_payload: e?.fal_request_payload ?? null };
                  }).filter((e) => e.url);
                  const selectedUrl = (sceneOrProject.background_image_url || "").trim();
                  const legacyIdx = selectedUrl ? legacyUrls.indexOf(selectedUrl) : -1;
                  const selected_index = legacyIdx >= 0 ? legacyIdx : 0;
                  return { image_urls: legacyUrls, selected_index, entries: legacyEntries };
                }

                const sceneState = useScenes && currentScene ? getSceneImageState(currentScene, true) : getSceneImageState(project, false);
                let entries = sceneState.entries;
                const selectedUrl = useScenes && currentScene
                  ? (sceneState.image_urls[sceneState.selected_index] || "").trim()
                  : (backgroundImageUrl || "").trim();
                if (entries.length === 0 && selectedUrl) {
                  entries = [{ url: selectedUrl, created_at: null, prompt_sent_to_model: null, model_endpoint: null, fal_request_payload: null }];
                }
                if (entries.length === 0 && !useScenes) return null;
                const sceneIndexForActions = safeSceneIndex;

                const handleDownloadBackgroundImage = async (url, index) => {
                  try {
                ...
                const handleDeleteBackgroundImage = async (url) => {
                  if (!(await confirm("Remove this generated image from the list and delete it from storage?"))) return;
                  if (!currentProjectId) return;
                  setDeletingBackgroundImageUrl(url);
                  try {
                    const body = { project_id: currentProjectId, url };
                    if (useScenes && currentScene?.id) body.scene_id = currentScene.id;
                    ...
                    const data = await res.json();
                    if (data.success) {
                      if (!useScenes && data.background_image_url !== undefined) setBackgroundImageUrl(data.background_image_url || "");
                      loadProject(currentProjectId);
                    } else {
                      await alert(data.error || "Failed to delete image", "error");
                    }
                  } catch (e) {
                    await alert(e.message || "Failed to delete image", "error");
                  } finally {
                  ...
                  <div className="rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900/80 p-5 shadow-sm">
                    {useScenes && (
                      <div className="flex flex-wrap items-center gap-2 mb-3">
                        <span className="text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">Scenes</span>
                        {scenes.map((s, i) => (
                          <button
                            key={s.id}
                            type="button"
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-2 mb-3">
                      <p className="text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">Generated images — select one for video{useScenes ? ` (${currentScene?.label || `Scene ${currentSceneIndex + 1}`})` : ""}</p>
                      <button
                        type="button"
                        onClick={() => setShowSceneImageGalleryModal(true)}
                    ...
                    <div className="flex flex-wrap gap-3">
                      {entries.map((entry, index) => {
                        ...
                        const isDeleting = deletingBackgroundImageUrl === url;
                        return (
                          <div
                            key={url}
                            onClick={async () => {
                              ...
                              try {
                                if (useScenes && currentScene?.id) {
                                  await fetch(`/api/quote-videos/projects/${currentProjectId}/scenes/${currentScene.id}`, {
                                    method: "PATCH",
                                    headers: { "Content-Type": "application/json" },
                                    body: JSON.stringify({ selected_image_index: index }),
                                  });
                                } else {
                                  await fetch(`/api/quote-videos/projects/${currentProjectId}`, {
                                    method: "PATCH",
                            ...
                            title={isSelected ? "Selected for video" : "Click to select for video"}
                          >
                            ...
                            <div className="absolute inset-0 bg-black/0 group-hover:bg-black/40 transition-colors flex flex-col justify-end p-1.5 gap-1">
                              <div className="flex flex-wrap gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                ...
                                <button
                                  ...
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    handleDownloadBackgroundImage(url, index);
                                  }}
                                  className="flex items-center gap-1 bg-black/80 text-white px-2 py-1 rounded text-xs hover:bg-black"
                                  title="Download"
                                >
                    ...
                    {(sceneState.entries[sceneState.selected_index]?.prompt_sent_to_model || currentScene?.image_prompt || (!useScenes && imagePromptUsed)) && (
                      <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-800">
                        <p className="text-xs font-medium text-gray-500 dark:text-gray-400 mb-2">Image prompt</p>
                        <div className="px-3 py-2 bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 rounded-lg text-sm text-gray-700 dark:text-gray-200 whitespace-pre-wrap break-words max-h-40 overflow-y-auto">
                          {useScenes
                            ? (sceneState.entries[sceneState.selected_index]?.prompt_sent_to_model || currentScene?.image_prompt || "No image prompt yet.")
                    ...
                    {selectedUrl && (
                      <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-800">
                        <p className="text-xs font-medium text-gray-500 dark:text-gray-400 mb-2">Selected for video</p>
                    ...
                    {selectedUrl && (
                      <div className="mt-4 p-4 rounded-xl border border-gray-200 dark:border-gray-700 bg-slate-50/50 dark:bg-slate-900/30">
                        <p className="text-xs font-semibold text-slate-700 dark:text-slate-300 mb-3 uppercase tracking-wide">Apply film grain</p>
                        <p className="text-xs text-slate-600 dark:text-slate-400 mb-3">Add a new grained version of the selected image (original unchanged). Uses <a href="https://fal.ai/models/fal-ai/post-processing/grain" target="_blank" rel="noopener noreferrer" className="text-blue-600 dark:text-blue-400 underline">fal.ai post-processing/grain</a>.</p>
                        <div className="flex flex-wrap items-center gap-4 mb-3">
                          <div className="flex items-center gap-2">
                            <label className="text-xs font-medium text-slate-700 dark:text-slate-300 whitespace-nowrap">Style</label>
                            <select
                              value={grainStyle}
                              onChange={(e) => setGrainStyle(e.target.value)}
                          ...
                          <div className="flex items-center gap-2">
                            <label className="text-xs font-medium text-slate-700 dark:text-slate-300 whitespace-nowrap">Intensity</label>
                            <input
                              type="range"
                              min={0}
                              max={1}
                            ...
                            />
                            <span className="text-xs tabular-nums text-slate-600 dark:text-slate-400 w-8">{grainIntensity}</span>
                          </div>
                          <div className="flex items-center gap-2">
                            <label className="text-xs font-medium text-slate-700 dark:text-slate-300 whitespace-nowrap">Scale</label>
                            <input
                              type="number"
                              min={1}
                              max={100}
                              value={grainScale}
                              onChange={(e) => setGrainScale(Math.max(1, Math.min(100, Number(e.target.value) || 10)))}
                        ...
                        <button
                          type="button"
                          onClick={() => handleApplyGrain(selectedUrl, useScenes && currentScene?.id ? currentScene.id : null)}
                        ...
                        >
                          {applyingGrain ? "Applying grain…" : "Apply grain"}
                        </button>
                      </div>
                    )}
                    ...
                    {backgroundImageInfoUrl && (() => {
                      const entry = entries.find((e) => e.url === backgroundImageInfoUrl);
                      const hasMeta = entry && (entry.prompt_sent_to_model != null || entry.model_endpoint != null || entry.fal_request_payload != null);
                      return (
                        <div
                          className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4"
                          onClick={() => setBackgroundImageInfoUrl(null)}
                          role="dialog"
                          aria-modal="true"
                        >
                          <div
                          ...
                          >
                            <div className="flex items-start justify-between gap-4 px-5 py-4 border-b border-gray-200 dark:border-gray-800">
                              <div className="min-w-0">
                                ...
                                <div className="text-xs text-gray-600 dark:text-gray-400 mt-1 truncate">URL: {backgroundImageInfoUrl}</div>
                              </div>
                            ...
                            <div className="px-5 py-4 overflow-y-auto space-y-4">
                              {!hasMeta && (
                                <div className="rounded-xl border border-amber-200 bg-amber-50 text-amber-900 p-4 text-sm dark:border-amber-900/50 dark:bg-amber-950/30 dark:text-amber-200">
                                  No generation metadata for this image (e.g. generated before metadata was stored).
                                </div>
                              )}
                              {entry?.created_at && (
                                <div className="rounded-2xl border border-gray-200 dark:border-gray-800 bg-white/70 dark:bg-gray-950/40 p-4">
                                  <div className="text-xs font-semibold text-gray-800 dark:text-gray-100 mb-2">Settings</div>
                                  <div className="text-xs text-gray-700 dark:text-gray-200 space-y-1">
                                    <div><span className="font-medium">Generated:</span> {new Date(entry.created_at).toISOString()}</div>
                                    {entry.model_endpoint && (
                                      <div><span className="font-medium">Model endpoint:</span> {entry.model_endpoint}</div>
                                    )}
                                  </div>
                                </div>
                              )}
                              {entry?.prompt_sent_to_model != null && (
                                <details className="rounded-2xl border border-gray-200 dark:border-gray-800 bg-white/70 dark:bg-gray-950/40 p-4" open>
                                  <summary className="cursor-pointer text-xs font-semibold text-gray-800 dark:text-gray-100">Prompt sent to model</summary>
                                  <pre className="mt-3 text-xs font-mono leading-relaxed text-gray-800 dark:text-gray-200 whitespace-pre-wrap break-words rounded-xl bg-gray-50/80 dark:bg-gray-900/40 border border-gray-200/80 dark:border-gray-800/80 p-3">
                                    {entry.prompt_sent_to_model || "(empty)"}
                                  </pre>
                                </details>
                              )}
                              {entry?.fal_request_payload != null && (
                                <details className="rounded-2xl border border-gray-200 dark:border-gray-800 bg-white/70 dark:bg-gray-950/40 p-4" open>
                                  <summary className="cursor-pointer text-xs font-semibold text-gray-800 dark:text-gray-100">Request payload (exact API params)</summary>
                                  <pre className="mt-3 text-[11px] font-mono leading-relaxed text-gray-800 dark:text-gray-200 whitespace-pre-wrap break-words rounded-xl bg-gray-50/80 dark:bg-gray-900/40 border border-gray-200/80 dark:border-gray-800/80 p-3">
                                    {JSON.stringify(entry.fal_request_payload, null, 2)}
                                  </pre>
                                </details>
                              )}
                            </div>
              ...
              <div className="flex gap-4">
                ...
                <button
                  ...
                  disabled={
                    !(backgroundImageUrl || "").trim() &&
                    (!Array.isArray(project?.scenes) || project.scenes.length === 0 || !project.scenes.some((s) => {
                      const urls = Array.isArray(s.image_urls) ? s.image_urls : [];
                      if (urls.length > 0) {
                        const idx = typeof s.selected_image_index === "number" ? s.selected_image_index : 0;
          ...
          {step === 3 && (
            <div className="space-y-6">
              <div className="mb-6">
                <h2 className="text-2xl font-bold mb-3 text-gray-900 dark:text-gray-100">Step 3: Generate video</h2>
                <div className="flex flex-wrap gap-2">
                  <div className="bg-purple-50 border border-purple-200 dark:border-purple-800 px-3 py-1.5 rounded-lg text-xs sm:text-sm font-medium flex items-center gap-1.5 whitespace-nowrap dark:bg-purple-950/40">
                    <span className="text-purple-700 dark:text-purple-200">🎬 Model:</span>
                    <span className="font-semibold text-purple-900 dark:text-purple-100" title={imageToVideoModel || ""}>
                      {imageToVideoModel ?? "…"}
                    </span>
                  </div>
                  {project?.costs?.step3?.fal > 0 && (
                    <div className="bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 px-3 py-1.5 rounded-lg text-xs sm:text-sm font-medium flex items-center gap-1.5 whitespace-nowrap">
                      <span className="text-gray-600 dark:text-gray-300">🎬 Fal:</span>
                      <span className="font-semibold text-gray-900 dark:text-gray-100">${project.costs.step3.fal.toFixed(4)}</span>
                    </div>
                  )}
                  {project?.costs?.step3?.total > 0 && (
                    <div className="bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 px-3 py-1.5 rounded-lg text-xs sm:text-sm font-medium flex items-center gap-1.5 whitespace-nowrap">
                      <span className="text-gray-600 dark:text-gray-300">💰 Step 3 Total:</span>
                      <span className="font-semibold text-gray-900 dark:text-gray-100">${project.costs.step3.total.toFixed(4)}</span>
                    </div>
                  )}
                </div>
              </div>
                ...
                <>
                  <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900/80 p-4 shadow-sm">
                    ...
                    <div className="h-2 w-full rounded-full bg-gray-200 dark:bg-gray-700 overflow-hidden">
                      <div
                        className="h-full rounded-full bg-blue-500 dark:bg-blue-500 min-w-[30%]"
                        style={{ animation: "quote-step3-loading-bar 1.5s ease-in-out infinite" }}
                      />
                    </div>
                  </div>
                  <style dangerouslySetInnerHTML={{ __html: `
                ...
                <div className="space-y-6">
                  <div className="flex flex-col gap-6">
                    {project.scenes.map((scene) => {
                      const selectedImageUrl = getSelectedImageUrlForScene(scene);
                      const selectedVideoUrl = (scene.selected_video_url || "").trim();
                      const sceneDuration = Math.max(1, Math.min(15, Math.round(Number(scene.duration) || 8)));
                      const isGenerating = generatingVideoSceneId === scene.id;
                      const canGenerate = Boolean(selectedImageUrl) && !isGenerating;
                      return (
                        <div
                          key={scene.id}
                        ...
                        >
                          ...
                          {(() => {
                            ...
                            const meta = Array.isArray(scene.image_metadata) ? scene.image_metadata : [];
                            const imagePromptText = (prompts[selIdx] && String(prompts[selIdx]).trim()) || meta[selIdx]?.prompt_sent_to_model || scene.image_prompt || "";
                            const videoPromptText = (scene.motion_prompt && String(scene.motion_prompt).trim()) || defaultMotionPrompt || "";
                            return (
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                <div>
                                  <div className="text-xs font-medium text-gray-700 dark:text-gray-200 mb-1">Image prompt</div>
                                ...
                                <div>
                                  <div className="flex items-center justify-between gap-2 mb-1">
                                    <span className="text-xs font-medium text-gray-700 dark:text-gray-200">Video prompt</span>
                                    <button
                                      type="button"
                                      onClick={() => handleGenerateVideoPromptForScene(scene)}
                                      disabled={generatingVideoPromptSceneId === scene.id || !imagePromptText.trim()}
                                    ...
                                    >
                                      {generatingVideoPromptSceneId === scene.id ? "Generating…" : "Generate from image prompt"}
                                    </button>
                                  </div>
                                  <div className="px-3 py-2 bg-white border border-gray-200 rounded-lg text-sm text-gray-700 dark:bg-gray-950 dark:border-gray-800 dark:text-gray-200 whitespace-pre-wrap break-words max-h-24 overflow-y-auto">
                                    {videoPromptText || "—"}
                          ...
                          <div className="flex flex-wrap items-center justify-between gap-2 mb-3">
                            <p className="text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">Generated videos — select one for this scene</p>
                            <div className="flex items-center gap-2">
                              <button
                                type="button"
                                onClick={() => handleGenerateVideoForScene(scene)}
                              ...
                              <button
                                type="button"
                                onClick={() => { setVideoHistoryTargetSceneId(scene.id); setShowVideoHistoryModal(true); }}
                                className="px-3 py-1.5 rounded-lg text-sm font-medium bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-gray-200 hover:bg-gray-300 dark:hover:bg-gray-600"
                              >
                                Select from history
                              </button>
                            ...
                            return (
                              <div className="flex flex-wrap gap-3 mb-5">
                                {videoEntries.map((entry, idx) => {
                                  const isSelected = (entry.url || "").trim() === (selectedVideoUrl || "").trim();
                                  return (
                                    <div
                                      key={(entry.url || "") + (entry.created_at || idx)}
                                      onClick={async () => { if (!isSelected) await handleSelectVideoFromHistoryForScene(entry, scene.id); }}
                                    ...
                                    >
                                      <video src={entry.url} className="w-full h-full object-cover" muted preload="metadata" playsInline />
                                      {isSelected && <div className="absolute top-1 left-1 bg-blue-500 text-white px-1.5 py-0.5 rounded text-xs font-medium">✓ Selected</div>}
                                      <div className="absolute bottom-1 left-1 right-1 text-[10px] text-white bg-black/70 rounded px-1 py-0.5 text-center">{entry.duration_seconds ?? 8}s</div>
                                    </div>
                                  );
                                })}
                                {videoEntries.length === 0 && !selectedVideoUrl && (
                                  <p className="text-sm text-gray-500 dark:text-gray-400 py-2">No videos yet. Generate one or select from history.</p>
                                )}
                              </div>
                            );
                          })()}
                          <div className="grid grid-cols-1 lg:grid-cols-[1fr_48px_1fr] gap-4 lg:gap-6 items-start">
                            <div>
                              <div className="text-xs font-medium text-gray-600 dark:text-gray-400 mb-1.5">Source image</div>
                              <div className="relative aspect-[9/16] max-h-[320px] rounded-lg overflow-hidden border border-gray-200 dark:border-gray-700 bg-gray-100 dark:bg-gray-900">
                                {selectedImageUrl ? (
                                  <>
                                    ...
                                    <button type="button" onClick={() => setExpandedImageUrl(selectedImageUrl)} className="absolute top-2 right-2 p-2 rounded-lg bg-black/60 hover:bg-black/80 text-white" title="Expand"><Maximize2 className="w-4 h-4" /></button>
                                  </>
                                ) : (
                                  <div className="w-full h-full flex items-center justify-center text-sm text-gray-500 dark:text-gray-400">No image</div>
                                )}
                              </div>
                            </div>
                            <div className="hidden lg:flex items-center justify-center text-gray-400 dark:text-gray-500 text-xl">→</div>
                            <div className="space-y-2">
                              <div className="text-xs font-medium text-gray-600 dark:text-gray-400">Selected clip</div>
                              <div className="relative aspect-[9/16] max-h-[320px] rounded-lg overflow-hidden border border-gray-200 dark:border-gray-700 bg-black">
                                {selectedVideoUrl ? (
                                  <>
                                    <video src={selectedVideoUrl} controls className="w-full h-full object-cover" preload="metadata" playsInline />
                                    <div className="absolute top-2 right-2 flex items-center gap-1">
                                      <button type="button" onClick={() => setExpandedVideoUrl(selectedVideoUrl)} className="p-2 rounded-lg bg-black/60 hover:bg-black/80 text-white" title="Expand"><Maximize2 className="w-4 h-4" /></button>
                                      <button type="button" onClick={() => setSceneVideoInfo({ sceneId: scene.id, label: scene.label || `Scene ${scene.id}`, metadata: scene.last_video_generation_metadata })} className="p-2 rounded-lg bg-black/60 hover:bg-black/80 text-white" title="Info"><Info className="w-4 h-4" /></button>
                                    </div>
                                  </>
                                ) : (
                                  <div className="w-full h-full flex items-center justify-center text-sm text-gray-500 dark:text-gray-400">{isGenerating ? "Generating…" : "No video yet"}</div>
                                )}
                              </div>
                              <div className="flex flex-wrap items-center gap-3">
                                <label className="text-xs font-medium text-gray-700 dark:text-gray-300">Duration (s)</label>
                                <input type="number" min={1} max={15} value={sceneDuration} onChange={(e) => updateSceneDuration(scene.id, Number(e.target.value) || 8)} className="w-16 px-2 py-1.5 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 text-sm" />
                              </div>
                            </div>
                          </div>
                        </div>
                  ...
                  <div className="rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900/80 p-5 shadow-sm space-y-4">
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <p className="text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">Generated videos — select one</p>
                      <div className="flex items-center gap-2">
                        <button
                          type="button"
                          onClick={handleGenerateVideo}
                          disabled={generatingVideo || (!backgroundImageUrl.trim() && !quoteText.trim() && !theme.trim())}
                          className="px-3 py-1.5 rounded-lg text-sm font-medium bg-blue-600 text-white hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                          {generatingVideo ? "Generating…" : "Generate video"}
                        </button>
                        <button type="button" onClick={() => setShowVideoHistoryModal(true)} className="px-3 py-1.5 rounded-lg text-sm font-medium bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-gray-200 hover:bg-gray-300 dark:hover:bg-gray-600">
                          Select from history
                        </button>
                      </div>
                    </div>
                    {(Array.isArray(project?.animation_video_history) && project.animation_video_history.length > 0) ? (
                      <div className="flex flex-wrap gap-3">
                        {[...project.animation_video_history].reverse().map((entry, i) => {
                          const isSelected = (entry.url || "").trim() === (animationVideoUrl || "").trim();
                          return (
                            <div
                              key={(entry.url || "") + (entry.created_at || i)}
                              onClick={async () => { if (!isSelected) await handleSelectVideoFromHistory(entry); }}
                      ...
                      <p className="text-sm text-gray-500 dark:text-gray-400">No videos yet. Generate one or select from history.</p>
                    )}
                    {animationVideoUrl && (
                      <div className="pt-4 border-t border-gray-200 dark:border-gray-800">
                        <p className="text-xs font-medium text-gray-500 dark:text-gray-400 mb-2">Selected clip</p>
                        <div className="flex flex-wrap gap-4 items-start">
                          <div className="relative aspect-[9/16] w-full max-w-sm rounded-lg overflow-hidden border border-gray-200 dark:border-gray-700 bg-black">
                            <video src={animationVideoUrl} controls className="w-full h-full object-cover" preload="metadata" playsInline />
                          </div>
                          <div className="flex flex-col gap-2">
                            <div className="flex items-center gap-2">
                              <label className="text-sm font-medium text-gray-700 dark:text-gray-300">Duration (s)</label>
                              <input type="number" min={5} max={60} value={durationSeconds} onChange={(e) => setDurationSeconds(Number(e.target.value) || 8)} className="w-20 px-2 py-1.5 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 text-sm" />
                            </div>
                            <div className="flex flex-wrap gap-2">
                              <button type="button" onClick={handleGenerateVideo} disabled={generatingVideo} className="text-sm text-blue-600 dark:text-blue-400 hover:underline disabled:opacity-50">Generate another</button>
                              <span className="text-gray-400 dark:text-gray-500">·</span>
                              <button type="button" onClick={() => setShowVideoHistoryModal(true)} className="text-sm text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-200">Select from history</button>
                            </div>
                          </div>
                        </div>
                      </div>
              ...
              <div className="flex gap-4">
                ...
                <button
                  type="button"
                  onClick={async () => { await saveStep2(); goToStep(4); }}
                  disabled={
                    Array.isArray(project?.scenes) && project.scenes.length > 0
                      ? !project.scenes.some((s) => (s.selected_video_url || "").trim())
          ...
          {step === 4 && (
            <div className="space-y-6">
              <div className="mb-6">
                <h2 className="text-2xl font-bold mb-3 text-gray-900 dark:text-gray-100">Step 4: Background Music</h2>
                <div className="flex flex-wrap gap-2">
                  <div className="bg-teal-50 border border-teal-200 dark:border-teal-800 px-3 py-1.5 rounded-lg text-xs sm:text-sm font-medium flex items-center gap-1.5 whitespace-nowrap dark:bg-teal-950/40">
                    <span className="text-teal-700 dark:text-teal-200">Video length:</span>
                    <span className="font-semibold text-teal-900 dark:text-teal-100">
                      {(() => {
                        const scenes = Array.isArray(project?.scenes) ? project.scenes : [];
                        const totalSec = scenes.length > 0
                          ? scenes.reduce((s, sc) => s + Math.max(1, Math.min(15, Math.round(Number(sc.duration) || 8))), 0)
                          : Math.max(5, Math.min(60, Number(durationSeconds) || 15));
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-violet-700 dark:text-violet-300 text-xs sm:text-sm font-medium">Model:</span>
                    <select
                      value={selectedMusicModelId}
                      onChange={(e) => { setSelectedMusicModelId(e.target.value); loadMusicPricing(e.target.value); }}
                  ...
                  <div className="bg-purple-50 dark:bg-purple-950/30 border border-purple-200 dark:border-purple-800 px-3 py-1.5 rounded-lg text-xs sm:text-sm font-medium flex items-center gap-1.5 whitespace-nowrap">
                    <span className="text-purple-700 dark:text-purple-200">Unit:</span>
                    <span className="font-semibold text-purple-900 dark:text-purple-100">
                      {loadingMusicPricing ? "..." : musicUnitCost != null ? `$${musicUnitCost.toFixed(2)}/${musicPricingUnit || "min"}` : "—"}
                    </span>
                  </div>
                  <div className="flex items-center gap-2">
                    <label htmlFor="quote-music-duration" className="text-xs sm:text-sm font-medium text-gray-700 dark:text-gray-200 whitespace-nowrap">
                      Duration (s):
                    </label>
                    <input
                      ...
                      onChange={(e) => setMusicDurationSeconds(Math.max(5, Math.min(600, Number(e.target.value) || 5)))}
                  ...
                  {project?.costs?.step4?.total > 0 && (
                    <div className="bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 px-3 py-1.5 rounded-lg text-xs sm:text-sm font-medium flex items-center gap-1.5 whitespace-nowrap">
                      <span className="text-gray-600 dark:text-gray-300">Step 4 Total:</span>
                      <span className="font-semibold text-gray-900 dark:text-gray-100">${project.costs.step4.total.toFixed(4)}</span>
                    </div>
                  )}
                </div>
              </div>
              <p className="text-sm text-gray-700 dark:text-gray-300">
                Generate or select background music. It will be mixed under the clip when you render in the next step.
              </p>
              {musicBasePrompts.length > 0 && (
                <div className="space-y-2">
                  <div className="flex items-center justify-between gap-2 flex-wrap">
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-200">Base prompt library</label>
                    <a
                  ...
                  {baseMusicPrompt && (
                    <div className="rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50 p-3 space-y-3">
                      <p className="text-xs font-medium text-gray-500 dark:text-gray-400 mb-1.5">Selected base (used when music prompt field is empty)</p>
                      <p className="text-sm text-gray-700 dark:text-gray-300 whitespace-pre-wrap">{baseMusicPrompt}</p>
                      {(() => {
                        const selectedBase = musicBasePrompts.find((b) => b.id === selectedMusicBaseId);
                        const samples = selectedBase?.sample_music ?? (selectedBase?.sample_music_url ? [{ url: selectedBase.sample_music_url, id: selectedBase.sample_music_id || "" }] : []);
                        if (!samples.length) return null;
                        return (
                          <div className="pt-2 border-t border-gray-200 dark:border-gray-600 space-y-2">
                            <p className="text-xs font-medium text-gray-500 dark:text-gray-400 mb-1.5">Samples (generated with this base)</p>
                            {samples.map((s, i) => (
              ...
              <div>
                <div className="flex items-center justify-between gap-3 mb-2">
                  <label htmlFor="quote-music-prompt" className="text-sm font-medium text-gray-700 dark:text-gray-200">
                  ...
                  <button
                    ...
                    onClick={async () => {
                      ...
                      try {
                        const res = await fetch("/api/video-generator/generate-music-prompt", {
                          method: "POST",
                          headers: { "Content-Type": "application/json" },
                          body: JSON.stringify({
                        ...
                        if (result.success && result.prompt?.trim()) {
                          setMusicPromptReview({ previousPrompt: musicPrompt, newPrompt: result.prompt.trim() });
                        } else {
                          await alert("Failed to generate prompt: " + (result.error || "Unknown"), "error");
                        }
                      } catch (err) {
                        await alert("Error: " + err.message, "error");
                      } finally {
              ...
              <div>
                <label htmlFor="quote-music-negative" className="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-2">
              ...
              <div className="rounded-xl border border-violet-200 dark:border-violet-800 bg-violet-50/30 dark:bg-violet-950/20 p-4">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                  <div>
                    <div className="text-sm font-semibold text-violet-900 dark:text-violet-100 mb-1">Generate new music</div>
                    <p className="text-xs text-gray-600 dark:text-gray-400">Uses the selected model. Leave prompt empty to use base + your quote for variation; or enter a custom prompt.</p>
                  </div>
                  <button
                    type="button"
                    onClick={async () => {
                      ...
                      try {
                        const res = await fetch("/api/quote-videos/generate-background-music", {
                          method: "POST",
                          headers: { "Content-Type": "application/json" },
                          body: JSON.stringify({
                            project_id: currentProjectId,
                            ...
                            negative_prompt: musicNegativePrompt.trim() || null,
                            base_id: selectedMusicBaseId || null,
                            music_length_ms: Math.round(Math.max(5, Math.min(600, musicDurationSeconds)) * 1000),
                          }),
                        });
                        const result = await res.json();
                        if (result.success) {
                          ...
                          fetch("/api/quote-videos/music-base-prompts")
                            ...
                            .then((data) => {
                              if (data.success && Array.isArray(data.bases)) {
                                setMusicBasePrompts(data.bases);
                                const selected = data.bases.find((b) => b.id === selectedMusicBaseId) || data.bases[0];
                                if (selected) setBaseMusicPrompt(selected.prompt);
                              }
                            })
                            .catch(() => {});
                        } else {
                          await alert("Failed to generate music: " + (result.error || "Unknown"), "error");
                        }
                      } catch (err) {
                        await alert("Error: " + err.message, "error");
                      } finally {
                  ...
                  <div className="mt-4 pt-4 border-t border-violet-200 dark:border-violet-700">
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <span className="text-sm font-medium text-violet-900 dark:text-violet-100">
                        Generating with {getMusicModelById(selectedMusicModelId)?.name || selectedMusicModelId}…
                      </span>
                    </div>
                    <p className="text-xs text-violet-700 dark:text-violet-300">This may take 30–60 seconds.</p>
                  </div>
                )}
              </div>
              {musicUrl && (
                <div className="rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-950 p-4">
                  <div className="flex items-center gap-2 mb-2">
                    <span className="text-sm font-semibold text-gray-700 dark:text-gray-200">Current music</span>
                    {(() => {
                      const current = availableMusicFromCollection.find((x) => x.music_url === musicUrl);
                      return current ? (
                        <button
                          type="button"
                  ...
                  <audio controls src={musicUrl} className="w-full max-w-md h-9" />
                  <div className="mt-2">
                    <button
                      type="button"
                      onClick={async () => {
                        try {
                          const res = await fetch(`/api/quote-videos/projects/${currentProjectId}`, {
                            method: "PATCH",
                            headers: { "Content-Type": "application/json" },
                            body: JSON.stringify({ music_url: null, background_music_id: null }),
                          });
                          const data = await res.json();
                          if (data.success) {
                            setMusicUrl("");
              ...
              {musicPromptReview && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
                  <div className="bg-white dark:bg-gray-900 rounded-xl shadow-xl max-w-lg w-full p-6 space-y-4">
                    <div className="text-sm font-medium text-gray-900 dark:text-gray-100">Apply generated prompt?</div>
                    <p className="text-xs text-gray-600 dark:text-gray-400 whitespace-pre-wrap">{musicPromptReview.newPrompt}</p>
                    <div className="flex gap-3">
                      <button
                        type="button"
                        onClick={() => { setMusicPrompt(musicPromptReview.newPrompt); setMusicPromptReview(null); }}
          ...
          {step === 5 && (
            <div className="admin-card p-8 flex flex-col min-h-[calc(100vh-10rem)]">
              <div className="mb-4">
                <h2 className="text-2xl font-bold mb-2 text-gray-900 dark:text-gray-100">Step 5: Assemble & Final Video</h2>
                ...
                <div className="flex flex-wrap gap-2">
                  {project?.costs?.step5?.shotstack > 0 && (
                    <div className="bg-pink-50 dark:bg-pink-950/30 border border-pink-200 dark:border-pink-800 px-3 py-1.5 rounded-lg text-xs sm:text-sm font-medium flex items-center gap-1.5 whitespace-nowrap">
                      <span className="text-pink-700 dark:text-pink-200">🎞️ Shotstack:</span>
                      <span className="font-semibold text-pink-900 dark:text-pink-100">${project.costs.step5.shotstack.toFixed(4)}</span>
                    </div>
                  )}
                  {project?.costs?.step5?.total > 0 && (
                    <div className="bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 px-3 py-1.5 rounded-lg text-xs sm:text-sm font-medium flex items-center gap-1.5 whitespace-nowrap">
                      <span className="text-gray-600 dark:text-gray-300">💰 Step 5 Total:</span>
                      <span className="font-semibold text-gray-900 dark:text-gray-100">${project.costs.step5.total.toFixed(4)}</span>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="flex flex-1 flex-col min-h-0 gap-4">
                ...
                <div className="flex-1 min-h-0 w-full flex flex-col rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden bg-white dark:bg-gray-900/50">
                  ...
                  <div className="shrink-0 px-4 py-3 border-b border-gray-200 dark:border-gray-700 bg-amber-50/50 dark:bg-amber-950/20">
                    <p className="text-xs font-semibold text-amber-800 dark:text-amber-200 mb-2 uppercase tracking-wide">Grain overlay</p>
                    {localGrainsList.length > 0 && (
                      <div className="flex flex-col sm:flex-row sm:items-center gap-2 mb-2">
                        <label className="text-xs font-medium text-amber-800 dark:text-amber-200 whitespace-nowrap">Grain (Firebase Storage):</label>
                        <select
                          value={localGrainsList.find((item) => item?.url === grainOverlayUrl)?.url ?? ""}
                          onChange={async (e) => {
                        ...
                        >
                          ...
                          {localGrainsList.map((item) => {
                            ...
                            const url = typeof item === "object" ? item.url : `${typeof window !== "undefined" ? window.location.origin : ""}/grains/${encodeURIComponent(item)}`;
                            return (
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center gap-3 flex-wrap">
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        <label className="text-xs font-medium text-amber-800 dark:text-amber-200 whitespace-nowrap">Opacity (preview):</label>
                        <input
                          type="range"
                          min={0}
                          max={1}
                        ...
                        />
                        <span className="text-sm tabular-nums font-medium text-amber-900 dark:text-amber-100 w-10">{Math.round(grainOpacity * 100)}%</span>
                      </div>
                    </div>
                    <p className="text-xs text-amber-700/80 dark:text-amber-300/80 mt-1">
                      Preview below updates with opacity. Rendered video uses this opacity.
                    </p>
                    <p className="text-xs text-amber-600/90 dark:text-amber-400/90 mt-1">
                      <strong>Where to get a grain URL:</strong> Use the dropdown to pick a grain from Firebase Storage (bucket path <code className="bg-amber-100 dark:bg-amber-900/40 px-1 rounded">grains/</code>), or paste any public HTTPS URL. You can also set <code className="bg-amber-100 dark:bg-amber-900/40 px-1 rounded">QUOTE_VIDEOS_GRAIN_OVERLAY_URL</code> in <code className="bg-amber-100 dark:bg-amber-900/40 px-1 rounded">.env</code> for a project-wide default.
                    </p>
                  </div>
                  ...
                  <div className="shrink-0 px-4 py-3 border-b border-gray-200 dark:border-gray-700 bg-slate-50/50 dark:bg-slate-950/20">
                    <p className="text-xs font-semibold text-slate-700 dark:text-slate-300 mb-2 uppercase tracking-wide">Video darken</p>
                    <div className="flex flex-wrap items-center gap-4">
                      <div className="flex items-center gap-2">
                        <label className="text-xs font-medium text-slate-700 dark:text-slate-300 whitespace-nowrap">Filter:</label>
                        <select
                          value={timelineSettings?.videoFilter === "darken" ? "darken" : "none"}
                          onChange={async (e) => {
                            const v = e.target.value === "darken" ? "darken" : "none";
                            setTimelineSettings((prev) => ({ ...(prev || {}), videoFilter: v }));
                            if (currentProjectId) {
                              try {
                                await fetch(`/api/quote-videos/projects/${currentProjectId}`, {
                                  method: "PATCH",
                                  headers: { "Content-Type": "application/json" },
                                  body: JSON.stringify({
                                    timeline_settings: { ...(timelineSettings || {}), videoFilter: v },
                                  }),
                                });
                              } catch (_) {}
                            }
                      ...
                      {timelineSettings?.videoFilter === "darken" && (
                        <div className="flex items-center gap-2">
                          <label className="text-xs font-medium text-slate-700 dark:text-slate-300 whitespace-nowrap">Darkening:</label>
                          <input
                            type="range"
                            min={0}
                            max={100}
                            step={5}
                            value={(() => {
                              const opacity = typeof timelineSettings?.videoOpacity === "number" ? timelineSettings.videoOpacity : 1;
                              return Math.round(((1 - opacity) / 0.8) * 100);
                            })()}
                            onChange={(e) => {
                              const pct = Number(e.target.value);
                              const opacity = 1 - (pct / 100) * 0.8;
                              setTimelineSettings((prev) => ({ ...(prev || {}), videoOpacity: opacity }));
                            }}
                            onMouseUp={async (e) => {
                              if (!currentProjectId) return;
                              const pct = Number(e.target.value);
                              ...
                              try {
                                await fetch(`/api/quote-videos/projects/${currentProjectId}`, {
                                  ...
                                  headers: { "Content-Type": "application/json" },
                                  body: JSON.stringify({
                                    timeline_settings: { ...(timelineSettings || {}), videoOpacity: opacity },
                                  }),
                                });
                              } catch (_) {}
                            }}
                  ...
                  <div className="flex-1 min-h-[320px] w-full">
                    {(() => {
                      const scenes = Array.isArray(project?.scenes) ? project.scenes : [];
                      const scenesWithVideo = scenes.filter((s) => (s.selected_video_url || "").trim());
                      ...
                      const editorScenes = hasScenesWithVideo
                        ? scenesWithVideo.map((s, i) => ({ id: i, duration: s.duration }))
                        : animationVideoUrl
                          ? [{ id: 0 }]
                          : [];
                      const editorSceneDurations = hasScenesWithVideo
                        ? Object.fromEntries(
                            scenesWithVideo.map((s, i) => [
                              i,
                              Math.max(1, Math.min(60, Math.round(Number(s.duration) || 8))),
                            ])
                          )
                        : animationVideoUrl
                          ? { 0: durationSeconds }
                      ...
                      const getSceneDurationSecondsForEditor = (scene) => {
                        if (!scene) return 8;
                        const d = scene.duration != null ? Number(scene.duration) : null;
                        if (Number.isFinite(d)) return Math.max(1, Math.min(60, Math.round(d)));
                        if (!hasScenesWithVideo && scene.id === 0) return durationSeconds;
                        return 8;
                      };
                      ...
                      return hasVideo ? (
                        <TimelineEditor
                          ...
                          grainOverlayUrl={grainOverlayUrl.trim() || undefined}
                          ...
                          getSceneDurationSeconds={getSceneDurationSecondsForEditor}
                        ...
                        <div className="flex items-center justify-center h-full min-h-[320px] text-gray-500 dark:text-gray-400 text-sm p-6 text-center">
                          Complete Step 3 (Generate video) to use the timeline editor.
                        </div>
                      );
                    })()}
                  </div>
                  ...
                  <div className="flex flex-wrap items-center gap-3">
                    <button
                      ...
                      disabled={
                        assembling ||
                        !quoteText.trim() ||
                        ((Array.isArray(project?.scenes) ? project.scenes : []).filter((s) => (s.selected_video_url || "").trim()).length === 0 &&
                          !animationVideoUrl.trim())
                      }
                    ...
                    >
                      {assembling ? "Starting render…" : finalVideoUrl ? "Render Again" : "Render Final Video"}
                    </button>
                    <button
                      type="button"
                      onClick={() => goToStep(4)}
          ...
          {step === 6 && (
            <div className="rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900/80 p-6 sm:p-8 shadow-sm">
              <div className="mb-6">
                <h2 className="text-2xl font-bold mb-3 text-gray-900 dark:text-gray-100">Step 6: Post</h2>
                <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
              ...
              {finalVideoUrl && (
                <>
                  ...
                  <div className="mb-8">
                    <h3 className="text-lg font-semibold mb-4 text-gray-900 dark:text-gray-100">Post to Social Media</h3>
                    <div className="mb-4">
                          ...
                          <div className="space-y-4">
                            <div>
                              ...
                              <div className="flex gap-2 mb-1">
                                ...
                                <button
                                  ...
                                  disabled={!!generatingCaptionFor || !quoteText?.trim()}
                            ...
                            <div className="pt-2">
                              {instagramStatusLoading ? (
                                ...
                                <div className="flex flex-col gap-2">
                                  <button onClick={() => handlePostToSocial("instagram")} disabled={posting} className="flex items-center justify-center gap-3 bg-gradient-to-r from-purple-500 to-pink-500 text-white py-4 rounded-lg font-medium hover:opacity-90 disabled:opacity-50">
                                    <SiInstagram className="w-7 h-7" /> Post to Instagram
                                  </button>
                                  <div className="flex items-center gap-3 flex-wrap">
                                    <button type="button" onClick={handleSwitchInstagramAccount} disabled={instagramSwitchLoading} className="text-sm text-gray-600 hover:text-gray-800 dark:text-gray-400 dark:hover:text-gray-200 font-medium">
                                      {instagramSwitchLoading ? "Switching…" : "Switch account"}
                                    </button>
                                ...
                                <a href={"/api/auth/instagram?return_to=" + encodeURIComponent(returnToForOAuth)} className="flex items-center justify-center gap-3 bg-gradient-to-r from-purple-500 to-pink-500 text-white py-4 rounded-lg font-medium hover:opacity-90 no-underline">
                        ...
                        {socialTab === "youtube" && (
                          <div className="space-y-4">
                            <div>
                              ...
                              <div className="flex gap-2 mb-1">
                                <textarea value={youtubeCaption} onChange={(e) => setYoutubeCaption(e.target.value)} placeholder="Enter caption for YouTube" className="flex-1 min-h-[100px] px-4 py-3 border border-gray-200 dark:border-gray-700 rounded-lg text-sm bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 resize-y" />
                                <button type="button" onClick={() => handleGenerateCaption("youtube")} disabled={!!generatingCaptionFor || !quoteText?.trim()} className="self-start px-4 py-2 rounded-lg bg-red-600 text-white font-medium hover:bg-red-700 disabled:opacity-50 disabled:cursor-not-allowed whitespace-nowrap">
                            ...
                            <div className="pt-2">
                              {youtubeStatusLoading ? (
                                ...
                                <div className="flex flex-col gap-2">
                                  <button onClick={() => handlePostToSocial("youtube")} disabled={posting} className="flex items-center justify-center gap-3 bg-red-600 text-white py-4 rounded-lg font-medium hover:bg-red-700 disabled:opacity-50">
                                ...
                                <a href={"/api/auth/youtube?return_to=" + encodeURIComponent(returnToForOAuth)} className="flex items-center justify-center gap-3 bg-red-600 text-white py-4 rounded-lg font-medium hover:bg-red-700 no-underline">
                        ...
                        {socialTab === "tiktok" && (
                          <div className="space-y-4">
                            <div>
                              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Caption</label>
                              <div className="flex gap-2 mb-1">
                                <textarea value={tiktokCaption} onChange={(e) => setTiktokCaption(e.target.value)} placeholder="Enter caption for TikTok" className="flex-1 min-h-[100px] px-4 py-3 border border-gray-200 dark:border-gray-700 rounded-lg text-sm bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 resize-y" />
                                <button type="button" onClick={() => handleGenerateCaption("tiktok")} disabled={!!generatingCaptionFor || !quoteText?.trim()} className="self-start px-4 py-2 rounded-lg bg-black text-white font-medium hover:bg-gray-800 disabled:opacity-50 disabled:cursor-not-allowed whitespace-nowrap">
                            ...
                            <div className="pt-2">
                              <button onClick={() => handlePostToSocial("tiktok")} disabled={posting} className="w-full flex items-center justify-center gap-3 bg-black text-white py-4 rounded-lg font-medium hover:bg-gray-800 disabled:opacity-50">
      ...
      {(expandedImageUrl || expandedVideoUrl) && (
        <div
          ...
          onClick={() => { setExpandedImageUrl(null); setExpandedVideoUrl(null); }}
          role="dialog"
          aria-modal="true"
        ...
        >
          ...
          >
            <div className="px-4 py-3 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
              <h2 id="video-info-title" className="text-lg font-semibold text-gray-900 dark:text-gray-100">
              ...
              {sceneVideoInfo.metadata ? (
                <>
                  {sceneVideoInfo.metadata.timestamp && (
                    <p><span className="font-medium text-gray-500 dark:text-gray-400">Generated:</span> {new Date(sceneVideoInfo.metadata.timestamp).toLocaleString()}</p>
                  )}
                  {sceneVideoInfo.metadata.model_endpoint && (
                    <p><span className="font-medium text-gray-500 dark:text-gray-400">Model:</span> {sceneVideoInfo.metadata.model_endpoint}</p>
                  )}
                  {sceneVideoInfo.metadata.prompt_used != null && (
                    <div>
                      <p className="font-medium text-gray-500 dark:text-gray-400 mb-1">Prompt used</p>
                      <p className="whitespace-pre-wrap rounded bg-gray-50 dark:bg-gray-800 p-2 text-xs">{sceneVideoInfo.metadata.prompt_used}</p>
                    </div>
                  )}
                  {sceneVideoInfo.metadata.negative_prompt_used != null && sceneVideoInfo.metadata.negative_prompt_used !== "" && (
                    <div>
                      <p className="font-medium text-gray-500 dark:text-gray-400 mb-1">Avoid</p>
                      <p className="whitespace-pre-wrap rounded bg-gray-50 dark:bg-gray-800 p-2 text-xs">{sceneVideoInfo.metadata.negative_prompt_used}</p>
                    </div>
                  )}
                  {sceneVideoInfo.metadata.storage?.path && (
                    <p className="text-xs text-gray-500 dark:text-gray-400 truncate" title={sceneVideoInfo.metadata.storage.path}>Path: {sceneVideoInfo.metadata.storage.path}</p>
                  )}
                </>
              ) : (
                <p className="text-gray-500 dark:text-gray-400">No generation metadata for this video.</p>
              )}
            </div>
          </div>
        </div>
      ...
      <ReferenceGalleryModal
        ...
        currentReferenceUrl={referenceImageUrl || project?.reference_image_url}
      ...
      {videoPromptReview && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60" role="dialog" aria-modal="true" aria-labelledby="video-prompt-review-title">
          <div className="bg-white dark:bg-gray-900 rounded-xl shadow-xl max-w-lg w-full max-h-[90vh] flex flex-col border border-gray-200 dark:border-gray-700">
            <div className="px-4 py-3 border-b border-gray-200 dark:border-gray-700">
              <h2 id="video-prompt-review-title" className="text-lg font-semibold text-gray-900 dark:text-gray-100">Review video prompt</h2>
              {videoPromptReview.cost > 0 && (
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Cost: ${videoPromptReview.cost.toFixed(4)}</p>
              )}
            </div>
            ...
            <div className="px-4 py-3 border-t border-gray-200 dark:border-gray-700 flex justify-end gap-2">
              <button
                type="button"
                onClick={() => { setVideoPromptReview(null); setVideoPromptReviewDraft(""); }}
      ...
      <SceneImageGalleryModal
        ...
        currentSelectedUrl={(() => {
          const scenes = Array.isArray(project?.scenes) ? project.scenes : [];
          const useScenes = scenes.length > 0;
          const safeIdx = useScenes ? Math.min(currentSceneIndex, Math.max(0, scenes.length - 1)) : 0;
          const scene = useScenes ? scenes[safeIdx] : null;
          if (!scene) return backgroundImageUrl || "";
          const urls = Array.isArray(scene.image_urls) ? scene.image_urls : [];
          const idx = typeof scene.selected_image_index === "number" ? scene.selected_image_index : 0;
          return (urls[idx] || "").trim();
        })()}
        ...
        >
          ...
          >
            <div className="p-4 border-b border-gray-200 dark:border-gray-800 flex items-center justify-between">
              <h2 id="video-history-modal-title" className="text-lg font-semibold text-gray-900 dark:text-gray-100">
                Select from history
              </h2>
              <button
                type="button"
            ...
            <div className="p-4 overflow-y-auto flex-1">
              ...
              {globalVideoHistoryLoading ? (
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400">No video history yet. Generate a video to add entries here.</p>
              ) : (
                <div className="flex flex-wrap gap-4">
                  {(() => {
                    const currentUrl = videoHistoryTargetSceneId
                      ? (project?.scenes?.find((s) => s.id === videoHistoryTargetSceneId)?.selected_video_url || "").trim()
                      : (animationVideoUrl || "").trim();
                    const selected = globalVideoHistoryEntries.find((e) => (e.url || "").trim() === currentUrl);
                    const rest = globalVideoHistoryEntries.filter((e) => (e.url || "").trim() !== currentUrl);
                    const orderedEntries = selected ? [selected, ...rest] : globalVideoHistoryEntries;
                    return orderedEntries.map((entry, i) => {
                    const isCurrent = (entry.url || "").trim() === currentUrl;
                    const subtitle = [entry.project_name, entry.scene_label].filter(Boolean).join(" · ") || null;
                    return (
                      <div
                        key={entry.id || (entry.url || "") + (entry.created_at || i)}
                      ...
                      >
                        ...
                        <div className="p-3 bg-white dark:bg-gray-800">
                          <p className="text-xs text-gray-600 dark:text-gray-400">
                            ...
                            {subtitle && <span className="block text-[10px] opacity-80 mt-0.5 truncate" title={subtitle}>{subtitle}</span>}
                            {entry.created_at && (
                          ...
                          <button
                            ...
                            onClick={() => {
                              if (videoHistoryTargetSceneId) {
                                handleSelectVideoFromHistoryForScene(entry, videoHistoryTargetSceneId);
                              } else {
      ...
      {showMusicCollectionPicker && (
        ...
        >
          ...
          >
            <div className="p-4 border-b border-gray-200 dark:border-gray-800 flex items-center justify-between">
              <h2 id="music-collection-picker-title" className="text-lg font-semibold text-gray-900 dark:text-gray-100">
            ...
            <div className="p-4 overflow-y-auto flex-1">
              {loadingMusicCollection ? (
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400 py-8">
                  No music in collection for this project. Generate music in this step first.
                </p>
              ) : (
                <ul className="space-y-3">
                  {availableMusicFromCollection.map((m) => {
                    ...
                    const durationStr = m.duration_ms
                      ...
                      : null;
                    const promptPreview = (m.prompt || m.background_music_prompt || "").slice(0, 80);
                    return (
                      <li
                        key={m.id}
                        className={
                      ...
                      >
                        <div className="flex-1 min-w-0">
                          {durationStr && (
                            <span className="text-xs text-gray-500 dark:text-gray-400 mr-2">{durationStr}</span>
                          )}
                          <p className="text-sm text-gray-700 dark:text-gray-300 truncate" title={m.prompt || m.background_music_prompt}>
                            {promptPreview || "—"}
                          </p>
                        </div>
                        <div className="flex items-center gap-2">
                          {m.music_url && (
                            <audio src={m.music_url} controls className="h-9 max-w-[200px]" />
                          )}
                          <button
                            type="button"
                          ...
                          <button
                            ...
                            onClick={async () => {
                              try {
                                const res = await fetch("/api/quote-videos/select-music", {
                                  method: "POST",
                                  headers: { "Content-Type": "application/json" },
                                  body: JSON.stringify({ project_id: currentProjectId, music_id: m.id }),
                                });
                                const result = await res.json();
                                if (result.success && result.music_url) {
                                  ...
                                  await alert(result.error || "Failed to select music", "error");
                                }
                              } catch (err) {
                                await alert("Error: " + err.message, "error");
                              }
      ...
      {musicDetailsModal && (
        ...
        >
          ...
          >
            <div className="p-4 border-b border-gray-200 dark:border-gray-800 flex items-center justify-between">
              <h2 id="music-details-title" className="text-lg font-semibold text-gray-900 dark:text-gray-100">
                Music details
              </h2>
              <button
                type="button"
            ...
            <div className="p-4 overflow-y-auto space-y-4">
              {musicDetailsModal.duration_ms != null && (
                <div>
                  <p className="text-xs font-medium text-gray-500 dark:text-gray-400 mb-0.5">Duration</p>
              ...
              {musicDetailsModal.cost != null && (
                <div>
                  <p className="text-xs font-medium text-gray-500 dark:text-gray-400 mb-0.5">Cost</p>
                  <p className="text-sm text-gray-800 dark:text-gray-200">${Number(musicDetailsModal.cost).toFixed(4)}</p>
                </div>
              )}
              {musicDetailsModal.model_endpoint && (
                <div>
                  <p className="text-xs font-medium text-gray-500 dark:text-gray-400 mb-0.5">Model</p>
                  <p className="text-sm text-gray-800 dark:text-gray-200 break-all">{musicDetailsModal.model_endpoint}</p>
                </div>
              )}
              <div>
                <p className="text-xs font-medium text-gray-500 dark:text-gray-400 mb-0.5">Prompt</p>
                <p className="text-sm text-gray-800 dark:text-gray-200 whitespace-pre-wrap">
                  {musicDetailsModal.prompt || musicDetailsModal.background_music_prompt || "—"}
                </p>
              </div>
              {musicDetailsModal.negative_prompt && (
                <div>
                  <p className="text-xs font-medium text-gray-500 dark:text-gray-400 mb-0.5">Negative prompt</p>
                  <p className="text-sm text-gray-800 dark:text-gray-200 whitespace-pre-wrap">{musicDetailsModal.negative_prompt}</p>
                </div>
              )}
              {musicDetailsModal.music_url && (
                <div>
                  <p className="text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Preview</p>
                  <audio src={musicDetailsModal.music_url} controls className="w-full h-9" />
                </div>
              )}
            </div>
          </div>
          ...
          >
            ...
            <div className="p-4 overflow-auto flex-1">
              {lastRenderPayload ? (
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400">No payload available. Render a video to see the payload.</p>
              )}
            </div>
            <div className="p-4 border-t border-gray-200 dark:border-gray-800 flex justify-end gap-2">
              {lastRenderPayload && (
                <button
                  type="button"
                  onClick={() => {
                    navigator.clipboard.writeText(JSON.stringify(lastRenderPayload, null, 2));
