...
import { InstagramOAuthSdk, InstagramScope } from "@microfox/instagram-oauth";
...
function verifySignedStateAndGetReturnPath(stateParam, secret) {
  ...
  try {
    ...
    const payloadJson = Buffer.from(
      payloadB64.replace(/-/g, "+").replace(/_/g, "/") + pad(payloadB64.length),
      "base64",
    ).toString("utf8");
    const sig = Buffer.from(
      sigB64.replace(/-/g, "+").replace(/_/g, "/") + pad(sigB64.length),
      "base64",
    );
...
export async function GET(request) {
  ...
  const successUrl = redirectBase.includes("?")
    ? `${redirectBase}&instagram=connected`
    : `${redirectBase}?instagram=connected`;
  ...
  try {
    ...
    LOG({
      ...
      userId_length:
        typeof tokens?.userId === "string" ? tokens.userId.length : 0,
    });
  } catch (e) {
    LOG({
      step: "callback",
